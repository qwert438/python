...
class DataManager:
    ...
    def predict_task_duration(self, title="", priority=2):
        """基于历史完成情况的任务耗时预测（加分项）"""
        if not self.db_path:
            return None
        conn = sqlite3.connect(self.db_path)
            ...
            for hist_title, cost_sec in rows:
                ...
                if hist_lower:
                    ...
                    if similarity > 0.4:  # 相似度阈值
                        similar_costs.append(max(int(cost_sec or 0), 1))

        # 3. 如果有相似任务，用加权平均；否则用全局平均
        if similar_costs and len(similar_costs) >= 2:
            ...
            predicted = similar_costs[0]
            confidence = "low"
            source = "仅 1 个相似历史任务，参考有限"
        else:
...
class TaskDetailWindow:
    ...
    def create_widgets(self):
        ...
        # 加分项：基于历史完成情况的任务耗时预测
        field_label(7, "历史预测")
        predict_frame = tk.Frame(container, bg=COLORS["panel"])
        predict_frame.grid(row=7, column=1, sticky="ew", pady=8)
        ...
        self.predict_detail_label = tk.Label(
            predict_frame,
            text="",
            font=(FONT_FAMILY, 8),
            fg=COLORS["muted"],
        ...
        self.predict_detail_label.pack(anchor="w", pady=(2, 0))
        # 绑定输入事件，实时更新预测
        self.title_entry.bind("<KeyRelease>", lambda e: self.refresh_prediction())
        self.priority_var.trace_add("write", lambda *a: self.refresh_prediction())
        self.refresh_prediction()

        button_frame = tk.Frame(container, bg=COLORS["panel"])
        button_frame.grid(row=8, column=0, columnspan=2, sticky="e", pady=(24, 0))
        if self.task:
    ...
    def refresh_prediction(self):
        ...
        prediction = self.app.dm.predict_task_duration(title=title, priority=priority)

        if prediction is None:
            self.predict_label.config(
                text="暂无历史数据可供预测，完成更多任务后将自动学习",
                fg=COLORS["muted"],
            )
            self.predict_detail_label.config(text="")
            return
        ...
        confidence_color = {
            ...
            "low": COLORS["muted"],
        }.get(prediction["confidence"], COLORS["muted"])

        confidence_text = {"high": "较高", "medium": "一般", "low": "较低"}.get(
            prediction["confidence"], "较低"
        )

        self.predict_label.config(
            text=f"预测耗时：{prediction['predicted_text']}（置信度：{confidence_text}）",
            fg=confidence_color,
        )
        self.predict_detail_label.config(
            text=f"{prediction['source']} | 全局平均：{prediction['global_avg_text']} | 历史共 {prediction['total_history']} 条记录"
        )
